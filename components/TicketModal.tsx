...
interface TicketModalProps {
    ...
    resultsContext?: WinningResult[]; // Optional context for Admin view to calculate winnings
}
...
const TicketModal: React.FC<TicketModalProps> = ({ 
    ...
    // Determine if we should show the expanded admin layout
    const showAdminLayout = variant === 'admin' && isConfirmed;
        ...
        <div ref={modalRef} className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50 animate-fade-in" onClick={onClose}>
            ...
            >
                ...
                <div className="flex-grow min-h-0 overflow-y-auto overscroll-contain bg-gray-50 dark:bg-black/20">
                    <div className={`h-full ${showAdminLayout ? 'grid grid-cols-1 md:grid-cols-2' : ''}`}>
                        ...
                        <div className={`flex justify-center p-4 ${showAdminLayout ? 'border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700' : ''}`}>
                            <div ref={ticketContentRef} className="bg-white p-3 text-black font-mono text-xs w-full max-w-[320px] mx-auto leading-normal shadow-sm">
                                ...
                                <div className="space-y-2 mb-3 text-[11px]">
                                     ...
                                     <p><span className="font-bold">TRACKS</span><br/>{selectedTracks.filter(t => t !== 'Pulito' && t !== 'Venezuela' && t !== 'New York Horses').join(', ')}</p>
                                </div>
                        ...
                        {/* RIGHT COLUMN: DIGITAL TWIN (ADMIN VIEW) - ONLY SHOW IF ADMIN VARIANT */}
                        {showAdminLayout && (
                            <div className="p-4 md:p-6 bg-slate-900 overflow-y-auto">
                                <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
                                        ...
                                        <p className="text-white font-bold truncate">{selectedTracks[0]}</p>
                                    </div>
                                    <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
                    ...
                    {isConfirmed ? (
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3 w-full">
                                ...
                                {/* Renamed "Share / Print" to "Share" per request */}
